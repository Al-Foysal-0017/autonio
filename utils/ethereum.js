 ...
 * @returns
 */
export const fetchLockedBalance = async (account, signer) => {
 ...
 * @returns
 */
export const fetchAvailableBalance = async (account, signer) => {
  if (!account || !signer) return;
 ...
 * @returns
 */
export const fetchDailyRewardRate = async () => {
  const tokenContract = new ethers.Contract(
    ContractAddress.STAKING_REWARDS,
    StakingRewardsABI,
    DEFAULT_PROVIDER
  ...
  const balance = await tokenContract.rewardRate();
